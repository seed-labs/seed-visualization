...
import type { GroundStation, SatelliteGroundLink, SatellitePoint, SimulationSettings } from '@/features/starlink/types';
...
const { now } = useSimulationClock(() => settings.speed);
...
const renderTime = computed(() =>
  !settings.useLocalGroundLinks && backendGroundLinkTime.value ? backendGroundLinkTime.value : now.value,
);
...
function removeBackendSatellite(satelliteId: string) {
  ...
  backendGroundLinks.value = backendGroundLinks.value.filter((link) => link.satelliteId !== satelliteId);
  backendLinkedSatelliteIds.value = backendLinkedSatelliteIds.value.filter((id) => id !== satelliteId);
}
