...
Examples:
  python satellite-emulator-service/scripts/create_ground_links_request.py --timestamp now --interval 10s --count 6 --out ground-links.json
  python satellite-emulator-service/scripts/create_ground_links_request.py --timestamp 1719123456789 --interval 10s --count 6 --post-url http://127.0.0.1:9091/api/v1/satellite/links
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        help="Write JSON payload to this file. With --post-url, defaults to satellite-emulator-service/tmp/links.json.",
    )
    ...
    if args.post_url:
        ...
        try:
            ...
            raise ValueError("The posted links file must be inside satellite-emulator-service/tmp.") from error
        if not request_path.startswith("tmp/"):
            raise ValueError("The posted links file must be inside satellite-emulator-service/tmp.")
